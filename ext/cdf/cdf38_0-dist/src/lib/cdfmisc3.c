...
*                               is not set. 
*   V3.4  12-Feb-17, M Liu      Added check TMPDIR for Unix-based and TEMP 
*                               for Windows if CDF_TMP is not set in 
*                               ScratchDirectory.
*
******************************************************************************/
...
void *buffer;
{
  ...
  ****************************************************************************/
  ...
  if (STRINGdataType(Var->dataType)) {
    FillNulsToString((char *)buffer, (int) numElems,
                     (int) Var->NvalueElems);
  }
  /****************************************************************************
			   ...
			   return values in host computer's encoding. */
{
  ...
  CDFstatus status; long dataType2, numElems2; void *filled = NULL;
  /****************************************************************************
  * Determine pad value.
  ****************************************************************************/
  ...
  status = GetFILLVALEntry(CDF, Var->zVar, Var->varN, &dataType2, &numElems2,
                           &filled);
  if (status == CDF_OK) {
    memcpy (padValue, filled, (size_t) Var->NvalueBytes);
  } else {
  ...
  if (filled != NULL) cdf_FreeMemory (filled, NULL);
  /****************************************************************************
...
#endif
#if defined(unix)
   if (envTmp == NULL) envTmp = getenv("TMPDIR");
#endif
#if defined(win32)
   if (envTmp == NULL) envTmp = getenv ("TMP");
   if (envTmp == NULL) envTmp = getenv ("TEMP");
#endif
  return envTmp;
}
...
VISIBLE_PREFIX int UTF8StrLength (string, max)
...
CDFstatus GetFILLVALEntry (id, Z, varNum, dataType, numElems, value)
CDFid id;
Logical Z;
long varNum;
long *dataType;
...
void **value;
{
  CDFstatus status, status2, status3;
  long dataType1, dataType2, numElems1, numElems2;
  CDFid id2; long curNum, curAttr, curEntry; int created;
  id2 = (CDFid) -1;
  curNum = curAttr = curEntry = -1L;
  status2 = CDFlib (CONFIRM_, CDF_, &id2,
                    NULL_);
  status2 = CDFlib (CONFIRM_, BOO(Z,zVAR_,rVAR_), &curNum,
                    NULL_);
  status2 = CDFlib (CONFIRM_, ATTR_, &curAttr,
                    NULL_);
  status2 = CDFlib (CONFIRM_, BOO(Z,zENTRY_,rENTRY_), &curEntry,
                    NULL_);
  status = CDFlib (SELECT_, CDF_, id,
                            BOO(Z,zVAR_,rVAR_), varNum,
                   GET_, BOO(Z,zVAR_DATATYPE_,rVAR_DATATYPE_), &dataType1,
                         BOO(Z,zVAR_NUMELEMS_,rVAR_NUMELEMS_), &numElems1,
                   NULL_);
  if (status != CDF_OK) {
    if (id2 != (CDFid) -1) 
      status2 = CDFlib (SELECT_, CDF_, id2,
                        NULL_);
    if (curNum != -1L)
      status2 = CDFlib (SELECT_, BOO(Z,zVAR_,rVAR_), curNum,
                        NULL_);
    return status;
  }
  ...
  if (status != CDF_OK) {
    ...
    status = CDFlib (SELECT_, ATTR_NAME_, "fillval",
                     NULL_);
    if (status != CDF_OK)
      /* Check for netCDF's _Fillval case */
      status = CDFlib (SELECT_, ATTR_NAME_, "_Fillval",
                       NULL_);
  }
  if (status != CDF_OK) {
    if (id2 != (CDFid) -1)
    ...
    if (curNum != -1L)
      status2 = CDFlib (SELECT_, BOO(Z,zVAR_,rVAR_), curNum,
                        NULL_);
    if (curAttr != -1L)
      status2 = CDFlib (SELECT_, ATTR_, curAttr,
                        NULL_);
    return status;
  }
  status = CDFlib (SELECT_, BOO(Z,zENTRY_,rENTRY_), varNum,
                   GET_, BOO(Z,zENTRY_DATATYPE_,rENTRY_DATATYPE_), &dataType2,
                         BOO(Z,zENTRY_NUMELEMS_,rENTRY_NUMELEMS_), &numElems2,
                   NULL_);
  if (status != CDF_OK) {
    if (id2 != (CDFid) -1)
      status2 = CDFlib (SELECT_, CDF_, id2,
    ...
    if (curAttr != -1L)
      status2 = CDFlib (SELECT_, ATTR_, curAttr,
                        NULL_);
    if (curEntry != -1L)
      status2 = CDFlib (SELECT_, BOO(Z,zENTRY_,rENTRY_), curEntry,
                        NULL_);
    return status;
  }
  if (*value == NULL) {
    int extra;
    if (STRINGdataType(dataType2)) extra = 1;
    else extra = 0;
    *value = (void *) cdf_AllocateMemory (
                (size_t) ((CDFelemSize(dataType2)+extra) * numElems2), NULL);
    created = 1;
  } else
    created = 0;
  status = CDFlib (GET_, BOO(Z,zENTRY_DATA_,rENTRY_DATA_), *value,
                   NULL_);
  if (status != CDF_OK) {
    if (created == 1) cdf_FreeMemory (*value, NULL);
  } else
    if (STRINGdataType(dataType2)) *((char *)*value+numElems2) = (char) '\0';
  if (id2 != (CDFid) -1)
    status2 = CDFlib (SELECT_, CDF_, id2,
                      NULL_);
  if (curNum != -1L)
  ...
  *dataType = dataType2;
  *numElems = numElems2;
  if (!EquivalentDataTypes (dataType1, dataType2)) return BAD_DATA_TYPE;
  else return CDF_OK;
}
