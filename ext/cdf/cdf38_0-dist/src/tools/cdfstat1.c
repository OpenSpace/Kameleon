...
Logical HandleVirtual (id, Var)
CDFid id;
struct VarStructST *Var;
{
  CDFstatus status;
  struct VarStructST VirtualVar;
  ...
  ****************************************************************************/
  if (ignoreFills) {
    ...
    status = GetFILLVALEntry (id, Var->Z, VirtualVar.varN,  &dataTypeE,
			      &numElemsE, &(VirtualVar.fillval));
    if (status != CDF_OK) {
      if (status == BAD_DATA_TYPE)
	cdf_FreeMemory (VirtualVar.fillval, FatalError);
...
/*
    ...
    if (status == NO_SUCH_ENTRY)
      ...
      VirtualVar.fillval = cdf_AllocateMemory (
                           (size_t) (CDFelemSize(VirtualVar.dataTypeV) *
                                     VirtualVar.numElemsV), FatalError);
