...
*   V3.4  14-Feb-17, M Liu      Never truncate the variable name when 
*                               displaying data values on screen to to file.
*
******************************************************************************/
...
Logical ToScreenHori () {
  ...
  static struct EditWindowStruct EWscr = {
    " Screen ListingH ", 0, 0, SCREEN_WIDTH, 1, header, screenLines,
  ...
  ****************************************************************************/
  ...
  for (Item = exportHead; Item != NULL; Item = Item->nextExport) {
	 ...
	 case RECORDt:
	   ...
	   break;
	 case VARIABLEt: {
	   int varWidth = MAXIMUM(((Item->Var->nRecordValues * Item->width) +
				   (opt.spacing *
                                    (Item->Var->nRecordValues-1))),
                                  (int)strlen(Item->Var->name));
  ...
  if (opt.textHeading) {
    ...
    for (Item = exportHead; Item != NULL; Item = Item->nextExport) {
	   ...
	   case VARIABLEt: {
	     ...
	     int varWidth = MAXIMUM(((Item->Var->nRecordValues*valueWidth) +
				     (opt.spacing*
				      (Item->Var->nRecordValues-1))),
                                    (int)strlen(Item->Var->name));
...
Logical ToFileCDAweb () {
  long addL, lineL = 0; int filterStatus;
  char *line = NULL, *line2 = NULL; long firstRec = 0, lastRec = 0, recN, nRecs;
          ...
          if (Item->Var->dataType == CDF_EPOCH) {
            for (ia = 0; ia < (int) Item->Var->nRecordValues; ++ia) {
              CatToString (line, Item->Var->name, valueWidth,
                           CENTER_JUSTIFY, "*");
              CatToString (line2, (ziso8601?"yyyy-mm-ddThh:mm:ss.mscZ":
                                            "yyyy-mm-ddThh:mm:ss.msc"),
                           valueWidth, CENTER_JUSTIFY, "*");
/*
...
*/
              CatToString (line2, (ziso8601?
                                   "yyyy-mm-ddThh:mm:ss.mmm.uuu.nnn.pppZ":
	    ...
	    ******************************************************************/
		...
		if (!cdaweb || 
		    (Item->Var->dataType != CDF_EPOCH && 
		     Item->Var->dataType != CDF_EPOCH16 &&
                     Item->Var->dataType != CDF_TIME_TT2000)) {
                  if (FLOAT4dataType(Item->Var->dataType)) {
                    if (!isnan((double)*(float *)Item->Var->value) &&
                      ...
                      EncodeValuesFormat (Item->Var->dataType,
                                          Item->Var->numElems,
                                          Item->Var->value, EofS(line),
                                          format, width, width, style,
                                          widthW-strlen(line));
                  } else if (DOUBLEdataType(Item->Var->dataType)) {
                    if (!isnan(*(double *)Item->Var->value) &&
                        (*(double *)Item->Var->value <=
                      ...
                      EncodeValuesFormat (Item->Var->dataType,
                                          ...
                                          format, width, width, style,
                                          widthW-strlen(line));
                  } else if (Item->Var->dataType == CDF_EPOCH) {
                    if (!isnan(*(double *)Item->Var->value) &&
                        *(double *)Item->Var->value <= DEFAULT_EPOCH_PADVALUE) {
                      EncodeValue (Item->Var->dataType, Item->Var->value,
                                   EofS(line), style,
                                   (size_t) widthW-strlen(line));
                    } else
                    ...
                    if (!isnan(*(double *)Item->Var->value) &&
                        !isnan(*(((double *)Item->Var->value)+1)) &&
                        (*(double *)Item->Var->value <= 
                         DEFAULT_EPOCH16_PADVALUE) &&
                        (*(((double *)Item->Var->value)+1) <=
                         DEFAULT_EPOCH16_PADVALUE)) {
                      EncodeValue (Item->Var->dataType, Item->Var->value,
                                   EofS(line), style,
                                   (size_t) widthW-strlen(line));
		  ...
		  if (Item->Var->dataType == CDF_EPOCH) {
		    char epString[EPOCH_STRING_LEN+1];
                    if (!isnan(*(double *)Item->Var->value)) {
                      toEncodeEPOCH(*(double *)Item->Var->value, (ziso8601?3:4),
                                    epString);
		      CatToString (EofS(line), epString,
                                   (ziso8601?EPOCH3_STRING_LEN:
                                             EPOCH4_STRING_LEN),
				   LEFT_JUSTIFY, "*");
                    } else {
                      strcpy(epString, "NaN");
		      CatToString (EofS(line), epString, 3, LEFT_JUSTIFY, "*");
                    }
		  } else if (Item->Var->dataType == CDF_EPOCH16) {
		    ...
		    time2[1] = *(((double *) Item->Var->value)+1);
                    if (isnan(time2[0]) || isnan(time2[1])) {
                      CatToString (EofS(line), "NaN", 3, LEFT_JUSTIFY, "*");
                    } else {
                      toEncodeEPOCH16(time2, (ziso8601?3:4), ep16String1);
		      CatToString (EofS(line), ep16String1, 
			  	   (ziso8601?EPOCH16_3_STRING_LEN:
                                             EPOCH16_4_STRING_LEN),
                                   LEFT_JUSTIFY, "*");
                    }
		  } else if (Item->Var->dataType == CDF_TIME_TT2000) {
		    char epString[TT2000_4_STRING_LEN+1];
		    encodeTT2000 (*(long long *)Item->Var->value,
		                  epString, (ziso8601?4:3));
		    CatToString (EofS(line), epString,
                                 (ziso8601?TT2000_4_STRING_LEN:
                                           TT2000_3_STRING_LEN),
				 LEFT_JUSTIFY, "*");
                  }
		}
         ...
         if (Item->Var->dataType != CDF_EPOCH &&
           ...
           if (qaFlags[idx] != NULL && qaFlags[idx][recN] > 0) {
                ...
                if (FLOAT4dataType(Item->Var->dataType)) {
                  ...
                  if (!isnan(*(double *)Item->Var->fill) &&
                      *(double *)Item->Var->fill <= DEFAULT_EPOCH16_PADVALUE) {
                    EncodeValue (Item->Var->dataType, Item->Var->fill,
                                 EofS(line), style,
                                 (size_t) widthW-strlen(line));
                  } else
		 ...
		 if (cdaweb)
		   width = MAXIMUM(width, strlen(Item->Var->name));
                 if (FLOAT4dataType(Item->Var->dataType)) {
                   ...
                   if (!isnan(*(double *)Item->Var->value) &&
                       *(double *)Item->Var->value <= DEFAULT_EPOCH_PADVALUE)
                     EncodeValue (CDF_EPOCH, Item->Var->value,
                                  EofS(line), style,
                                  (size_t) widthW-strlen(line));
                   else
                     EncodeValuesFormat (CDF_EPOCH, Item->Var->numElems,
                                         Item->Var->value, EofS(line),
                                         format, width, width, style,
                                         widthW-strlen(line));
                 } else if (Item->Var->dataType == CDF_EPOCH16) {
                   if (!isnan(*(double *)Item->Var->value) &&
                       ...
                       (*(((double *)Item->Var->value)+1) <=
                        DEFAULT_EPOCH16_PADVALUE))
                     EncodeValue (CDF_EPOCH16, Item->Var->value,
                                  EofS(line), style,
                                  (size_t) widthW-strlen(line));
                   else
                     EncodeValuesFormat (CDF_EPOCH16, Item->Var->numElems,
                                         Item->Var->value, EofS(line),
                                         format, width, width, style,
                                         widthW-strlen(line));
                 } else {
...
Logical ToScreenVert () {
  ...
  static int exitKeys[] = { NUL };
  static struct EditWindowStruct EWscr = {
    " Screen ListingV ", 0, 0, SCREEN_WIDTH, 1, header, screenLines,
    NUMscreenLINES, 1, NULL, FALSE, TRUE, exitKeys, REFRESHkey_FSI, NUL, NUL,
    NUL, NUL, NUL, NUL, NUL, NUL, NUL
  };
  ...
  ****************************************************************************/
  for (Item = exportHead; Item != NULL; Item = Item->nextExport) {
     if (Item->output) {
       ...
       * Determine if enough room.
       ***********************************************************************/
       if (Item->type == RECORDt || Item->type == INDICESt)
         addL = BOO(totalWidth == 0,0,opt.spacing) + Item->width;
       else
         addL = BOO(totalWidth == 0,0,opt.spacing) + 
                MAXIMUM(Item->width,(int)strlen(Item->Var->name));
       if (totalWidth + addL <= lineL) {
	 totalWidth += addL;
	 if (Item->type == VARIABLEt) {
  ...
  ****************************************************************************/
  if (noRoom > 0) {
    ...
    snprintf (msg, (size_t) sizeof(msg), "Not enough room for %d item%s.",
	      noRoom, BOO(noRoom <= 1,"","s"));
    DisplayMessage (msg, NOBEEPWAIT2);
  }
  /****************************************************************************
  * Check if a valid listing.
  ****************************************************************************/
  if (totalWidth == 0) {
    DisplayMessage ("Nothing selected for listing.", BEEPWAIT2);
    return TRUE;
  }
  /****************************************************************************
  * Remove items from the export list that are not being output or filtered.
  ...
  ****************************************************************************/
  ...
  if (range == 1L) {
    if (!AdjustFirstLastRecords(&firstRec, &lastRec)) return FALSE;
    if (firstRec == -1) {
      DisplayMessage ("No records found for listing.", BEEPWAIT2);
      return TRUE;
    }
  } else if (range == 2L) {
    if (firstRec < recordStart) firstRec = recordStart;
  ...
  ****************************************************************************/
  ...
  for (Item = exportHead; Item != NULL; Item = Item->nextExport) {
	 ...
	 case INDICESt:
	   ...
	   break;
	 case VARIABLEt:
	   CatToString (header[0], Item->Var->name,
                        MAXIMUM(Item->width, (int)strlen(Item->Var->name)),
			CENTER_JUSTIFY, "*");
	   break;
       }
...
Logical ToFileVert () {
  ...
  ****************************************************************************/
  for (Item = exportHead; Item != NULL; Item = Item->nextExport) {
	 ...
	 case VARIABLEt: {
           int width, addL;
	   width = BOO(simpleMode,
		       StandardWidth(Item->Var->dataType,
				     Item->Var->numElems),Item->width);
	   addL = BOO(lineL == 0,0,opt.spacing) + 
		  MAXIMUM(width,(int)strlen(Item->Var->name));
	   lineL += addL;
    ...
    for (Item = exportHead; Item != NULL; Item = Item->nextExport) {
	   ...
	   case VARIABLEt: {
	     int standardWidth = StandardWidth(Item->Var->dataType,
					       Item->Var->numElems);
	     int valueWidth = BOO(simpleMode,standardWidth,Item->width);
	     CatToString (line, Item->Var->name, 
                          MAXIMUM(valueWidth,(int)strlen(Item->Var->name)),
			  CENTER_JUSTIFY, "*");
	     break;
	   }
         }
  ...
  ****************************************************************************/
  for (Item = exportHead; Item != NULL; Item = Item->nextExport) {
	 ...
	 *********************************************************************/
	 ...
	 int standardWidth = StandardWidth(Item->Var->dataType,
					   Item->Var->numElems);
	 int width = BOO(standard,standardWidth,
                         MAXIMUM(Item->width,(int)strlen(Item->Var->name)));
	 char *format = BOO(standard,
			    StandardFormat(Item->Var->dataType),
			    Item->Var->format);
	     ...
	     CatNcharacters (line, width, '-');
             ...
             if (FLOAT4dataType(Item->Var->dataType)) {
               if (!isnan((double)*(float *)Item->Var->value) &&
                   *(float *)Item->Var->value <= DEFAULT_FLOAT_PADVALUE) {
                 EncodeValue (Item->Var->dataType, Item->Var->value,
                              EofS(line), style, (size_t) widthW-strlen(line));
               } else
	         EncodeValuesFormat (Item->Var->dataType, Item->Var->numElems,
		  		     Item->Var->value, EofS(line), format,
				     width, width, style, widthW-strlen(line));
             } else if (DOUBLEdataType(Item->Var->dataType)) {
               if (!isnan(*(double *)Item->Var->value) &&
                   *(double *)Item->Var->value <= DEFAULT_DOUBLE_PADVALUE) {
                 EncodeValue (Item->Var->dataType, Item->Var->value,
...
struct ItemStruct *exportHead;
{
  ...
  ****************************************************************************/
  ...
  handles = (Byte1 ***) cdf_AllocateMemory ((size_t)scalarCount *
                                            sizeof(Byte1 **),
				            FatalError);
  nValueBytes = (size_t *) cdf_AllocateMemory ((size_t)scalarCount *
                                               sizeof(size_t),
					       FatalError);
  ...
  for (Item = exportHead, nrvAt = 0; Item != NULL; Item = Item->nextExport) {
     if (NRVtoOUTPUT(Item) && Item->Var->maxRec == 0) {
	  ...
	  nRecordValues *= phyDimSizes[dimN];
       ...
       handles = (Byte1 ***) cdf_AllocateMemory ((size_t)sizeof(Byte1 **),
                                                 FatalError);
       nValueBytes = (size_t *) cdf_AllocateMemory ((size_t)sizeof(size_t),
                                                    FatalError);
       handles[0] = &buffer;
...
long numElems;
{
  switch (dataType) {
    ...
    case CDF_TIME_TT2000:
      switch (opt.epochStyle) {
        case EPOCH0_STYLE: return TT2000_0_STRING_LEN;
        case EPOCH1_STYLE: return TT2000_1_STRING_LEN;
        case EPOCH2_STYLE: return TT2000_2_STRING_LEN;
        case EPOCH3_STYLE: return TT2000_4_STRING_LEN;
