  ...
  ****************************************************************************/
  switch (argC) {
	...
	"include", "exclude", "sequencetime", "controlsettings", 
        "epochrange", "recordrange", "ziso8601", NULL
      };
        ...
        if (includeVars > 0 && qop->qualEntered[SETTINGSqual]) {
          ...
          if (includeVars > 0) {
            if ((strstrIgCase(tmp, "varsfile=") != NULL) ||
              ...
              if (numVars > 0) {
                int i;
                vars = (char **) cdf_AllocateMemory ((size_t)sizeof(char *) *
                                                     numVars, FatalError);
                for (i = 0; i < numVars; ++i)
                  vars[i] = (char *) cdf_AllocateMemory (
                                    (size_t)CDF_VAR_NAME_LEN256+1, FatalError);
	...
	if (!strcmp(qop->qualOpt[ZMODEqual],"0"))
      ...
      /************************************************************************
      * Check for `ziso8601' qualifier.
      ************************************************************************/
      if (qop->qualEntered[ZISO8601qual]) {
	  ziso8601 = TRUE;
      }	else
	  ziso8601 = FALSE;
      if (!ziso8601) opt.epochStyle = DEFAULTstyleEXPORT;
      else opt.epochStyle = EPOCH3_STYLE;
      /************************************************************************
	...
	static char *validOptions[] = {
	  ...
	  "noheading", "iso8601", "poundedheading", "ziso8601", NULL
	};
	/**********************************************************************
	...
	while (*option != NUL) {
	  ...
	  ********************************************************************/
	  ...
	  switch (which) {
	    ...
	    case ZISO8601opt: opt.epochStyle = EPOCH3_STYLE; break;
...
Logical LoadCDF (CDFid id) {
  ...
  ****************************************************************************/
  ...
  if (outputVars == 2L) {
    vars =
     (char **) cdf_AllocateMemory ((size_t)sizeof(char *) * nVars, FatalError);
  ...
  ****************************************************************************/
  for (varX = 0; varX < nVars; varX++) {
     ...
     *************************************************************************/
     ...
     Item->inclusive = TRUE;
     Item->Var = (struct VarStructXP *) cdf_AllocateMemory (
                                            (size_t)sizeof(struct VarStructXP),
					    FatalError);
     Item->nextItem = NULL;
     if (itemHead == NULL)
       itemHead = Item;
     ...
     *************************************************************************/
     if (opt.useFILLVAL) {
       status = GetFILLVALEntry (id, Item->Var->zVar, Item->Var->varN,
				 &eDataType, &eNumElems, &(Item->Var->fill));
       if (StatusBAD(status)) {
	 ...
	 *********************************************************************/
	 ...
	 switch (Item->type) {
	   ...
	   case INDICESt:
	     ...
	     break;
	   case VARIABLEt:
             if (TT2000dataType(Item->Var->dataType)) {
               if (!ziso8601) style = opt.epochStyle;
	     ...
	     if (Item->Var->min != NULL)
	       EncodeValuesFormat (Item->Var->dataType, Item->Var->numElems,
				   ...
				   Item->Var->format, -MINitemLEN, MINitemLEN,
				   style,
				   (size_t) SCREEN_WIDTH-2-
					    strlen(IWsel->iLines[lineN]));
	     else
	       CatNcharacters (IWsel->iLines[lineN], MINitemLEN, ' ');
	     break;
	 }
	   ...
	   case VARIABLEt:
	     ...
	     if (Item->Var->max != NULL)
	       EncodeValuesFormat (Item->Var->dataType, Item->Var->numElems,
				   ...
				   Item->Var->format, -MAXitemLEN, MAXitemLEN,
				   style,
				   (size_t) SCREEN_WIDTH-2-
					    strlen(IWsel->iLines[lineN]));
	     else
	 ...
	 switch (Item->type) {
	   ...
	   case VARIABLEt:
	     ...
	     if (Item->Var->fill != NULL) {
	       ...
	       EncodeValuesFormat (Item->Var->dataType, Item->Var->numElems,
				   Item->Var->fill, EofS(temp),
				   Item->Var->format, -MAXitemLEN,
				   FILLitemLEN - 2, style,
...
void OptionMenu () {
  ...
  static long nextEncoding[] = {
    ...
    IBMPC_ENCODING, IBMRS_ENCODING, HOST_ENCODING, PPC_ENCODING,
    HP_ENCODING, 0L, NeXT_ENCODING, ALPHAOSF1_ENCODING, ALPHAVMSd_ENCODING,
    ALPHAVMSg_ENCODING, ALPHAVMSi_ENCODING, ARM_LITTLE_ENCODING,
    ARM_BIG_ENCODING, IA64VMSi_ENCODING, IA64VMSd_ENCODING,
    IA64VMSg_ENCODING, NETWORK_ENCODING
  };
  static long prevEncoding[] = {
    0L, IA64VMSg_ENCODING, NETWORK_ENCODING, SUN_ENCODING,
        VAX_ENCODING, DECSTATION_ENCODING, SGi_ENCODING, IBMPC_ENCODING,
        IBMRS_ENCODING, HOST_ENCODING, 0L, PPC_ENCODING, HP_ENCODING,
        NeXT_ENCODING, ALPHAOSF1_ENCODING, ALPHAVMSd_ENCODING,
        ALPHAVMSg_ENCODING, ALPHAVMSi_ENCODING, ARM_LITTLE_ENCODING,
        ARM_BIG_ENCODING, IA64VMSi_ENCODING, IA64VMSd_ENCODING 
  };
     ...
     *************************************************************************/
	 ...
	 switch (IWopt.itemN) {
	   ...
	   case EPOCHrx:
	     ...
	     if (style != opt.epochStyle) {
	       long dt; int ix; struct ItemStruct *itemx;
	       if (opt.epochStyle == EPOCH3_STYLE) ziso8601 = TRUE;
               else ziso8601 = FALSE;
	       itemx = itemHead;
	       while (itemx != NULL) {
	         if (itemx->type == VARIABLEt) {
	           dt = itemx->Var->dataType;
	           if (dt == CDF_EPOCH || dt == CDF_EPOCH16 ||
		       dt == CDF_TIME_TT2000) {
	             itemx->width = VariableWidth(itemx->Var);
	           }
	         }
	         itemx = itemx->nextItem;
	       }
	     }
	     break;
	   case SPACINGrx:
  ...
  switch (Item->type) {
    ...
    **************************************************************************/
	...
	EncodeValuesFormat (Item->Var->dataType, Item->Var->numElems, *mmPtr,
			    value, Item->Var->format, 0, MAXminMaxLEN, style,
			    (size_t) sizeof(value));
      /************************************************************************
      * Prompt for new value.
      ************************************************************************/
	...
	if (LTx(newValue,Item->Var->min,
      ...
      ************************************************************************/
      if (*mmPtr == NULL)
	*mmPtr = cdf_AllocateMemory ((size_t)Item->Var->nValueBytes,
				     FatalError);
...
struct VarStructXP *Var;
{
  ...
  ****************************************************************************/
  if (Var->fill == NULL)
    ...
    EncodeValuesFormat (Var->dataType, Var->numElems, Var->fill, value,
			Var->format, 0, MAXfillLEN, style,
			(size_t) sizeof(value));
  /****************************************************************************
  * Prompt for new value.
  ****************************************************************************/
...
struct VarStructXP *Var;
{
  ...
  ****************************************************************************/
    ...
    switch (opt.epochStyle) {
      case EPOCH0_STYLE:
        return MAXIMUM(TT2000_0_STRING_LEN,MIN_VARIABLE_WIDTH);
      case EPOCH1_STYLE:
        return MAXIMUM(TT2000_1_STRING_LEN,MIN_VARIABLE_WIDTH);
      case EPOCH2_STYLE:
        return MAXIMUM(TT2000_2_STRING_LEN,MIN_VARIABLE_WIDTH);
      case EPOCH3_STYLE:
        return MAXIMUM(TT2000_4_STRING_LEN,MIN_VARIABLE_WIDTH);
      case EPOCH4_STYLE:
...
void OptDefaults () {
  if (simpleMode) {
    ...
    if (!ziso8601)
      opt.epochStyle = DEFAULTstyleEXPORT;
    else
      opt.epochStyle = EPOCH3_STYLE;
...
char **epochStrings;
{
  ...
  if (len1 == 24 || len1 == 23) {
    epochStart = toParseEPOCH (epochStrings[0]);
    epochEnd = toParseEPOCH (epochStrings[1]);
  } else if (len1 == 36 || len1 == 32) {
    epochTemp = toParseEPOCH16 (epochStrings[0], epoch16Start);
    epochTemp = toParseEPOCH16 (epochStrings[1], epoch16End);
  } else {
    tt2000Start = toParseTT2000 (epochStrings[0]);
    tt2000End = toParseTT2000 (epochStrings[1]);
  }
  return TRUE;
}
