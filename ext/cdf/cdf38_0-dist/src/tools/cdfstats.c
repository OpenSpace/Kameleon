...
char *argV[];
{
  ...
  long numVars, numZvars, varN, zMode = DEFAULTzModeSTATS;
  ...
  static char *validQuals[] = {
    ...
    "statistics", "nostatistics", "epoch_monotonic", "ziso8601", NULL
  };
...
struct VarStructST *Var;
{
  ...
  if (Var->varMaxRec == -1 && epochMonotonic) {
    ...
    **************************************************************************/
    if (virtual == 1) {
      ...
      if (!HandleVirtual(id, Var)) return FALSE;
        return TRUE;
    } else {
  ...
  ****************************************************************************/
  if (ignoreFills) {
    status = GetFILLVALEntry (id, Var->Z, Var->varN, &dataTypeE, &numElemsE,
			      &(Var->fillval));
    if (status != CDF_OK) {
      if (status == BAD_DATA_TYPE) cdf_FreeMemory (Var->fillval, FatalError);
