...
char tt2000[TT2000_4_STRING_LEN+1];
...
int chr;
{
  ...
  switch (searchFor) {
    ...
    **************************************************************************/
	...
	catchrX (epoch, chr, EPOCH_STRING_LEN);
	epochCount++;
        if (epochCount == 3 && StrStrIgCaseX(epoch, "nan")) {
          if (!ItemStream(epoch)) return FALSE;
        } else {
	  if ((lineIds+epochCount) == lineLen)
	    if (!ItemStream(epoch)) return FALSE;
        }
      }
      break;
    ...
    **************************************************************************/
    case VAREPOCH16:
	  ...
	  lineIds = lineIdx;
        ...
        catchrX (epoch2, chr, EPOCH16_STRING_LEN);
        epochCount++;
        if (epochCount == 3 && StrStrIgCaseX(epoch2, "nan")) {
    ...
    **************************************************************************/
    case VARTT2000:
      if (epochCount == 0) {
        if (!Spacing(chr)) {
          catchrX (tt2000, chr, TT2000_4_STRING_LEN);
          epochCount = 1;
	  lineIds = lineIdx;
        }
      }
...
char *item;
{
  CDFstatus status; Logical zVar = TRUE;
  ...
  long numDims = 0, varNum, dimSizes[CDF_MAX_DIMS];
  long *dimVarysP = NULL;  /* Points to the `dimVarys' array to be used. */
  ...
  switch (nextItem) {
    ...
    **************************************************************************/
    case CDFCOMPRESSIONfield: {
      ...
      strcpyX (cmpStr, item, (size_t) (MAX_SKTLINE_LEN+1));
    ...
    **************************************************************************/
	...
	eNumElems = strlen (item);
        if (eNumElems != 0) valuesB = item;
        else {
          eNumElems = 1L;
          allocated = 1;
          valuesB = (void *) malloc (1);
          *(char *)valuesB = (char) '\0';
        }
      }
    ...
    **************************************************************************/
	...
	if (BOO(zSection,zNumDims,rNumDims) > 0) {
	  ...
	  if (!StatusHandlerS2C(status,BOO(zVar,creZ,creR))) return FALSE;
          if (mLog) {
            char tempS[256+1];
            snprintf (tempS, 256, "*** created var=%s\n",varName);
            WriteOut (stdout, tempS);
          }
	  /********************************************************************
	  * Check if a pad value may possibly be written with the value of the
	  * FILLVAL (or _Fillval) attribute.  If so, we want to allocate space
	  * for it now so
    ...
    **************************************************************************/
	...
	eNumElems = strlen (item);
        if (eNumElems != 0) eValueB = item;
        else {
          eNumElems = 1L;
          allocated = 1;
          eValueB = (void *) malloc (1);
          *(char *)eValueB = (char) '\0';
        }
      }
      zVar = BOO(zSection,TRUE,(zMode > zMODEoff));
	...
	if (!strcmpITB(vAttrName,"FILLVAL") ||
	    (strcmpIgCase(vAttrName,"fillval") == 1) ||
	    (strcmpIgCase(vAttrName,"_fillval") == 1)) {
	  nBytes = (size_t) (vNumElems * CDFelemSize(vDataType));
	  padValue = cdf_AllocateMemory (nBytes, FatalError);
    ...
    **************************************************************************/
	...
	NextSearchItem (TOKEN, VaNAMEfieldOrPERIODmark);
      ...
      status = CDFlib (PUT_, VAR_BLOCKINGFACTOR(zVar), bf,
                       NULL_);
      if (!StatusHandlerS2C(status,blkVAR)) return FALSE; 
      break;
    }
    /**************************************************************************
    *
    ...
    **************************************************************************/
    case VARPADVALUEfield: {
      ...
      if (!STRINGdataType(vDataType)) {
        ...
        strcpyX (tmp, valuesB, (size_t) vNumElems);
        *(tmp+vNumElems) = '\0';
        len = (int) strlen (valuesB);
        if ((len-1) < vNumElems) {
          int ix;
          for (ix = (len-1); ix < vNumElems; ++ix) *(tmp+ix) = (char) '\0';
        }
        zVar = BOO(zSection,TRUE,(zMode > zMODEoff));
    ...
    **************************************************************************/
	...
	if (len > vNumElems) {
	  ...
	  value = item;
	...
	else if (len < vNumElems) {
	    if (mLog) {
	      char tempS[MAX_MSG_LEN+1];
	      WriteOut (stdout,
			"WARNING: Nul padding value for variable `");
	      WriteOut (stdout, varName);
	      WriteOut (stdout, "'\n");
	      snprintf (tempS, (size_t) sizeof(tempS),
			"         at line number %d.\n", lineN);
	    ...
	    NulPadRight (value, (int) (vNumElems - len));
	}
	else
	    value = item;
      }
