...
*   V3.7  10-Jan-18, M Liu      Added ARM and IA64 openVMS.
*   V3.7a 20-Jul-19, M Liu      Added "removedim" and "padtofilled" options.
*
******************************************************************************/
#include "cdfcvt.h"
#include "cdflib.h"
...
static CDFstatus ShuffleRecords (CDFid, Logical, struct HyperStruct, char *,
                                 long recSize);
static void AdjustTT2000Records (long count, char *, long recSize,
                                 int adjustLS);
static int SearchNeighbor (long, void *, void *, int *, int, int);
static void FindCDFPath (char *, char *, char *);
static void ReplacePadbyFilled (CDFid, CDFid, Logical, long,
                                struct HyperStruct, void *);
...
Logical sameFile = FALSE;
Logical removeDim = FALSE;
Logical padToFilled = FALSE;
...
#endif
#if defined(DEBUG)
  if (cdf_FreeMemory(NULL,FatalError) > 0)
    DisplayWarning ("Abandoned buffers.");
#else
...
char *argV[];
{
  ...
  static char *validQuals[] = {
    ...
    "leapsecondlastupdated", "compressnonepoch", "removedim", "padtofilled",
    NULL };
  ...
  switch (argC) {
	...
	if (sscanf(newfield, "%d", &lastUpdated) != 1) {
      ...
      ************************************************************************/
      ...
      /************************************************************************
      * Check for removedim qualifier.
      ************************************************************************/
      if (qop->qualEntered[REMOVEDIMqual]) {
        removeDim = TRUE;
      }
      /************************************************************************
      * Check for padtofilled qualifier.
      ************************************************************************/
      if (qop->qualEntered[PADTOFILLEDqual]) {
        padToFilled = TRUE;
      }
      /************************************************************************
	...
	if (dstEncoding != SOURCEencoding) {
	  ...
	  static char *encodingStrings[] = {
	    ...
	    "ibmpc", "ibmrs", "ppc", "hp", "next", "alphaosf1",
	    "alphavmsd", "alphavmsg", "alphavmsi", "arm_little",
            "arm_big", "ia64vmsi", "ia64vmsd", "ia64vmsg", NULL
	  };
	  static long encodings[] = {
	    ...
	    ALPHAVMSd_ENCODING, ALPHAVMSg_ENCODING, ALPHAVMSi_ENCODING,
            ARM_LITTLE_ENCODING, ARM_BIG_ENCODING, IA64VMSi_ENCODING,
            IA64VMSd_ENCODING, IA64VMSg_ENCODING
	  };
...
char *dstPath;
{
  ...
  char actualPathX[DU_MAX_PATH_LEN+1];
  char actualPathY[DU_MAX_PATH_LEN+1];
  char dstTmpPath[DU_MAX_PATH_LEN+1];
  int  valid;
  char extensionX[6+1], extensionY[6+1];
...
/*		   SELECT_, CDF_READONLY_MODE_, READONLYon, */
  ...
  ***************************************************************************/
  ...
  if (status == CDF_EXISTS && deleteIFexists) {
    char CDFnameX[DU_MAX_PATH_LEN+1], CDFnameY[DU_MAX_PATH_LEN+1];
    char CDFnameA[DU_MAX_PATH_LEN+1], CDFnameB[DU_MAX_PATH_LEN+1];
    char *ptr; int ilen;
    FindCDFPath (srcPath, CDFnameX, extensionX);
#if !defined(win32) && !defined(vms) && !defined(__MINGW32__)
    ptr = realpath(CDFnameX, actualPathX);
#else
# if defined(_WIN64) && !defined(__MINGW32__)
    /* Only found on 64-bit MS VS */
    GetFullPathNameA(CDFnameX, DU_MAX_PATH_LEN, actualPathX, &ptr);
# else
     strcpy(actualPathX, CDFnameX);
# endif
#endif
    FindCDFPath (dstPath, CDFnameY, extensionY);
#if !defined(win32) && !defined(vms) && !defined(__MINGW32__)
    ptr = realpath(CDFnameY, actualPathY);
#else
# if defined(_WIN64) && !defined(__MINGW32__)
    /* Only found on 64-bit MS VS */
    GetFullPathNameA(CDFnameY, DU_MAX_PATH_LEN, actualPathY, &ptr);
# else
    strcpy(actualPathY, CDFnameY);
# endif
#endif
#if defined(win32) || defined(vms)
    if (strcmpIgCase(actualPathX, actualPathY) == 1) sameFile = TRUE;
#else
    ptr = strstrIgCase (actualPathX, ".cdf");
    ilen = (int) strlen (actualPathX) - strlen(ptr);
    strcpyX (CDFnameA, actualPathX, (size_t)ilen);
    CDFnameA[ilen] = '\0';
    ptr = strstrIgCase (actualPathY, ".cdf");
    ilen = (int) strlen (actualPathY) - strlen(ptr);
    strcpyX (CDFnameB, actualPathY, (size_t)ilen);
    CDFnameB[ilen] = '\0';
    if (strcmp(CDFnameA, CDFnameB) == 0) sameFile = TRUE;
#endif
    if (!sameFile) { 
      ...
      char *last;
      srand(time(NULL));
      last = strstrIgCase (dstPath, ".cdf");
      if (last == NULL)  strcpy(dstTmpPath, dstPath); 
      else {
        int ilen = (int)strlen(dstPath)-strlen(last);
        strcpyX (dstTmpPath, dstPath, (size_t)ilen);
        *(dstTmpPath+ilen) = '\0';
      }
      sprintf(EofS(dstTmpPath), "%08d", rand()%100000000);
      sprintf(EofS(dstTmpPath), "%s", extensionX);

      status = CDFlib (CREATE_, CDF_, dstTmpPath, numDims, dimSizes, &dstId,
		       NULL_);
    }
  }
  if (!StatusHandlerCvt("DST",status)) {
...
/*			    COMPRESS_CACHESIZE_, compressCache, */
  ...
  ***************************************************************************/
  if (!backward && srcLastUpdated != -1 && lastUpdated == -1 &&
      adjustLS == -1) {
    status = CDFlib (SELECT_, CDF_, dstId,
  ...
  if (dumpStatistics) {
    ...
    valid = CDFgetValidate();
    if (valid == 1) CDFsetValidate (VALIDATEFILEoff);
    status = CDFlib (OPEN_, CDF_, srcPath, &srcId,
		     DELETE_, CDF_,
		     NULL_);
    StatusHandlerCvt ("SRC", status);
    ...
    DisplayStatistics ("destination", &vStatsDotCDF, &vStatsStage,
		       &vStatsCompress);
    if (valid == 1) CDFsetValidate (VALIDATEFILEon);
  }
  else {
    if (mLog) WriteOut (stdout, "  Closing source CDF...\n");
    if (!sameFile) {
  ...
  if (sameFile) rename (dstTmpPath, actualPathX);
...
int entryType;
{
  ...
  if (status != NO_SUCH_ENTRY) {
    if (rzEntry == 1) {
      status = CDFlib (SELECT_, (entryType == rENTRYt ? rVAR_ : zVAR_),
                                entryNum,
...
Logical HasDepend (cdfId, entryType, varNum, depend)
CDFid cdfId;
int entryType;
long varNum;
char *depend;
{
  CDFstatus status;
  long numAttrs, dataType, maxEntry, maxZentry, scope;
  char attrName[CDF_ATTR_NAME_LEN256+1];
  int  ix;

  status = CDFlib (SELECT_, CDF_, cdfId,
		   GET_, CDF_NUMATTRS_, &numAttrs,
			 ATTR_MAXrENTRY_, &maxEntry,
			 ATTR_MAXzENTRY_, &maxZentry,
                   NULL_);
  if (status != CDF_OK) return TRUE;
  for (ix = 0; ix < (int) numAttrs; ++ix) {
    status = CDFlib (SELECT_, ATTR_, (long) ix,
		     GET_, ATTR_NAME_, attrName,
			   ATTR_SCOPE_, &scope,
		     NULL_);
    if (!StatusHandlerCvt("SRC",status)) return TRUE;
    if (scope == GLOBAL_SCOPE) continue;
    if (strcmpIgCase(attrName, depend) == 0) continue;
    status = CDFlib (SELECT_, ENTRY(entryType), varNum,
		     GET_, ENTRY_DATATYPE(entryType), &dataType,
		     NULL_);
    if (status == CDF_OK) return TRUE;
    else return FALSE;
...
long DSTmajority;
{
   CDFstatus status; long cPct;
   long oVarNum, dataType, numElements, recVary, dimVarys[CDF_MAX_DIMS];
   long numDims, dimSizes[CDF_MAX_DIMS], NvalueBytes, maxRec, blockingFactor;
   long cType, cParms[CDF_MAX_PARMS], sRecordsType, reservePct;
...
/*					       &sArraysPct, */
   ...
   ***************************************************************************/
   ...
   if (Z) {
     if (!removeDim || numDims == 0L)
       status = CDFlib (SELECT_, CDF_, dstId,
			CREATE_, zVAR_, varName, dataTypeX, numElements, numDims,
					dimSizes, recVary, dimVarys, &oVarNum,
			NULL_);
     else {
       long dimSizesX[CDF_MAX_DIMS], dimVarysX[CDF_MAX_DIMS];
       int xy, xz;
       Logical hasDepend;
       hasDepend = FALSE;
       for (xy = xz = 0; xy < (int) numDims; ++xy) {
	 if (dimVarys[xy] && dimSizes[xy] > 1) {
	   dimVarysX[xz] = dimVarys[xy];
	   dimSizesX[xz++] = dimSizes[xy];
	 } else {
           char depend[9];
           snprintf(depend, 9, "depend_%d", (xy+1));
	   hasDepend = HasDepend (srcId, zENTRYt, varNum, depend);
	   if (hasDepend) break;
	 }
       }
       if (hasDepend) 
	 status = CDFlib (SELECT_, CDF_, dstId,
			  CREATE_, zVAR_, varName, dataTypeX, numElements,
	 ...
	 status = CDFlib (SELECT_, CDF_, dstId,
			  CREATE_, zVAR_, varName, dataTypeX, numElements,
					  (long) xz, dimSizesX, recVary,
					  dimVarysX, &oVarNum, NULL_);
     }
     if (!StatusHandlerCvt("DST",status)) return FALSE;
   }
   else {
   ...
   ***************************************************************************/
   ...
   if (maxBF == 1) {
       if (once == 0) {
         long maxRecX;
         status = CDFlib (SELECT_, CDF_, srcId,
                          GET_, VARs_MAXREC(Z), &maxRecX,
                          NULL_);
         if (!StatusHandlerCvt("DST",status)) return FALSE;
         once = 1;
         blockingFactorA = maxRecX + 1;
       }
   } else if (maxBF == 2) {
       if (recVary) {
	 long recSize = 1;
	     ...
	     CHECKforABORTso
             if (epochvsTT2000 == 0 || (dataType != CDF_EPOCH &&
               ...
               if (sortMax > 1 && sortMax == (maxRec+1) && recVary &&
                 ...
                 if (padToFilled && !CDFstringDataType(dataType))
                   ReplacePadbyFilled (srcId, dstId, Z, oVarNum, hyper,
                                       buffer2);
	       ...
	       if (!StatusHandlerCvt("DST",status)) return FALSE;
	         ...
	         for (ix = 0; ix < numDims; ++ix) values *= (int) dimSizes[ix];
                 values *= numElements;
	         bufferx = (long long *) cdf_AllocateMemory (
					(size_t) hyper.recCount *
					values * 8, FatalError);
	         if (bufferx != NULL) {
		   ...
		   for (ix = 0; ix < (int) count; ++ix) {
		     hyper.recNumber += ix;
                     ...
                     if (sortMax > 1 && sortMax == (maxRec+1) && recVary &&
                         sRecordsType == NO_SPARSERECORDS)
                       status = ShuffleRecords (dstId, Z, hyper,
                                                (char *) bufferx, values * 8);
                     else {
                       if (padToFilled && !CDFstringDataType(dataType))
                         ReplacePadbyFilled (srcId, dstId, Z, oVarNum, hyper,
                                             bufferx);
                 ...
                 for (ix = 0; ix < numDims; ++ix) values *= (int) dimSizes[ix];
                 values *= numElements;
                 bufferx = (double *) cdf_AllocateMemory (
					(size_t) hyper.recCount *
                                        values * 8 * step, FatalError);
                 if (bufferx != NULL) {
                   ...
                   bufferx = (double *) cdf_AllocateMemory (
					(size_t)values*8*step, FatalError);
                   if (bufferx == NULL) return FALSE;
                   count = hyper.recCount;
                   hyper.recCount = 1;
                   for (ix = 0; ix < (int) count; ++ix) {
	...
	for (hyperN = 0; hyperN < nHypers; hyperN++) {
	   ...
	   if (epochvsTT2000 == 0 || (dataType != CDF_EPOCH &&
             ...
             if (sortMax > 1 && sortMax == (maxRec+1) && recVary &&
               ...
               if (padToFilled && !CDFstringDataType(dataType))
                 ReplacePadbyFilled (srcId, dstId, Z, oVarNum, hyper, buffer1);
	       ...
	       for (ix = 0; ix < numDims; ++ix) values *= dimSizes[ix];
               values *= numElements;
	       bufferx = (long long *) cdf_AllocateMemory (
					(size_t)hyper.recCount * values * 8,
					FatalError);
	       if (bufferx != NULL) {
	         ConvertEpoch (dataType, hyper.recCount * values,
  ...
  for (;;) {
     ...
     *p2 = NUL;
     ...
     if (strcmpIgCase(p1,"var") == 1) {
       ...
       Var = (struct sRecordsVarStruct *) cdf_AllocateMemory (nBytes,
							      FatalError);
       Var->name = NULL;
       Var->next = S->VARhead;
       S->VARhead = Var;
...
static void ReplacePadbyFilled (srcId, dstId, zVar, varNum, hyper, buffer)
CDFid srcId;		/* IN: Source CDF id. */
CDFid dstId;		/* IN: Destination CDF id. */
Logical zVar;		/* IN: r/zVariable flag. */
long  varNum;    	/* IN: Variable id. */
struct HyperStruct hyper;/* IN: hyper structure. */
void  *buffer;		/* IN/OUT: The buffer. */ 
{
  long numDims, dimSizes[CDF_MAX_DIMS], dimVarys[CDF_MAX_DIMS], dataType,
       dataTypeSize, dataType2;
  int recCount = (int) hyper.recCount;
  int  ix, valCount;
  void *pad, *filled = NULL;
  CDFstatus status;
long dataType3, numElems3;
  status = CDFlib (SELECT_, CDF_, dstId,
                   ...
                   NULL_);
  status = CDFlib (GET_, DATATYPE_SIZE_, dataType, &dataTypeSize,
                   NULL_);
  pad = malloc ((size_t) dataTypeSize);
  status = CDFlib (GET_, BOO(zVar,zVAR_PADVALUE_,rVAR_PADVALUE_), pad,
                   NULL_);
  if (status != CDF_OK) GetDefaultPadValue(srcId, dataType, pad);
status = GetFILLVALEntry (dstId, zVar, varNum, &dataType3, &numElems3, &filled);
if (status != CDF_OK) {
  if (status == BAD_DATA_TYPE)
    cdf_FreeMemory (filled, FatalError);
  free (pad);
  return;
}
...
/*
  ...
  status = CDFlib (SELECT_, BOO(zVar,zENTRY_,rENTRY_), varNum,
                   GET_, BOO(zVar,zENTRY_DATATYPE_,rENTRY_DATATYPE_),
                         &dataType2,
                   NULL_);
  if (status != CDF_OK || !EquivDataTypes((Int32)dataType, (Int32)dataType2)) {
    free (pad);
    return;
  }
  filled = malloc ((size_t) dataTypeSize);
  status = CDFlib (GET_, BOO(zVar,zENTRY_DATA_,rENTRY_DATA_), filled,
                   NULL_);
  if (status != CDF_OK) {
    free (pad);
    free (filled);
...
*/
  ...
  for (ix = 0; ix < (int)numDims; ++ix)
    if (dimVarys[ix]) valCount *= (int) dimSizes[ix];
  ReplacePadValues (buffer, pad, filled, dataType, valCount);
  free (pad);
  cdf_FreeMemory (filled, FatalError);
}

static void FindCDFPath (path, fullPath, extension)
char *path;             /* IN: Base pathname. */
char *fullPath;         /* OUT: Path name with extension */
char *extension;	/* OUT: The extension in the path name */ 
{
  ...
  if (IsReg(path)) {
    strcpyX (fullPath, path, DU_MAX_PATH_LEN);
    ptr = strstrIgCase (path, ".cdf");
    strcpyX (extension, ptr, 6);
    return;
  }
  ...
  if (IsReg(pathT)) {
    strcpyX (fullPath, pathT, DU_MAX_PATH_LEN);
    strcpyX (extension, ".cdf", 6);
    return;
  }
