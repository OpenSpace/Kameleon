...
/*
      try {
        ...
        System.out.println("*** Error at setting the validation..."+vax);
        System.exit(1);
      }
*/
      for (ii = 0; ii < jj; ++ii) {
                ...
                for (Enumeration e1 =ent.elements() ; e1.hasMoreElements() ;) {
                    ...
                    if (entry != null) {
                          ...
                          if (eDataType == CDF_EPOCH) {
                              CDFUtils.printData (
                                Epoch.toEncode(((Double) data).doubleValue()));
                          } else if (eDataType == CDF_EPOCH16) {
                              CDFUtils.printData (Epoch16.toEncode((double[])data));
                          } else if (eDataType == CDF_TIME_TT2000) {
                              CDFUtils.printData (
                                CDFTT2000.toEncode(((Long)data).longValue()));
                          } else {
